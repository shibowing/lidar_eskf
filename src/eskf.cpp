...
void ESKF::update_imu(const sensor_msgs::Imu &msg) {
    ...
    if(_acc_queue_count < _acc_queue_size) {
        ...
        for(int i=0; i<_acc_queue_size; i++) {
         ...
         //这里定义的是50hz,因此5帧实际对应的是0.1秒，计算的是0.1秒内的平均加速度
    ...
    if(!_imu_enabled) {
        ...
        // If imu has quaternion, remove gravity.  如果用到IMU，就必须通过IMU移除加速度对应的重力
        if(_imu_has_quat) {
...
void ESKF::propagate_covariance() {
    ...
    // update covariance  //整个预测状态的协方差矩阵
...
void ESKF::update_error() {
    ...
    //建立观测方程，由于只得到pose的观测，因此是（x,y,z,roll,pitch,yaw）共6维
    ...
    // update covariance 更新协方差矩阵（完全跟公式一样）
